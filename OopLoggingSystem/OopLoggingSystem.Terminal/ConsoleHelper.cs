    ...
    public static class ConsoleHelper
    {
        public static int ReadNumber(string label, int maxAttempts = 3, int defaultValue = 0)
        {
            ...
            while (attempts <= maxAttempts)
            {
                ...
                {
                    attempts++;
                    Console.WriteLine($"Value '{input}' doesn't represent a valid number. Please try again ...");
                    Console.Write(label);
                }
            }

            string message = $"Exceeded max attempts count '{maxAttempts}', using {defaultValue}";
